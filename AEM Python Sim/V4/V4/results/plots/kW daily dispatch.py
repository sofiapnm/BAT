...
DAILY_OUTPUT_CSV_PATH = "/workspaces/BAT/AEM Python Sim/V4/V4/results/plots/kW daily dispatch.csv"


def load_daily_average_dataframe(csv_path):
    """Load the source CSV and aggregate all numeric series to daily means."""
...
def main():
    ...
    df = load_daily_average_dataframe(csv_path)
    df.to_csv(DAILY_OUTPUT_CSV_PATH, index=False)

    fig = build_figure(df, plot_label)
    fig.add_annotation(
        text=f"Source: {objective_mode.capitalize()}-optimized kWh results.csv, daily averages",
        xref="paper",
        yref="paper",
        x=0,
        y=-0.12,
    ...
    print(f"Wrote daily averages to {DAILY_OUTPUT_CSV_PATH}")
    print(f"Wrote plot to {HTML_OUTPUT_PATH}")
