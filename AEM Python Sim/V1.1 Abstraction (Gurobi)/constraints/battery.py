...
def add_battery_constraints(model, vars_dict, n, init_soc_kwh, final_soc_kwh):
    ...
    for t in range(n):
        model.addConstr(
            batt_charge[t] <= max_charge_kwh,
            name=f"charge_rate_limit[{t}]",
        )

        model.addConstr(
            batt_discharge[t] <= max_discharge_kwh,
            name=f"discharge_rate_limit[{t}]",
        )

        if t == 0:
            model.addConstr(
                soc[t] == init_soc_kwh + charge_eff * batt_charge[t] - batt_discharge[t],
                name=f"soc_transition[{t}]",
            )
        else:
            model.addConstr(
                soc[t] == soc[t - 1] + charge_eff * batt_charge[t] - batt_discharge[t],
                name=f"soc_transition[{t}]",
            )

    if final_soc_kwh is not None:
        model.addConstr(
            soc[n - 1] == final_soc_kwh,
            name="final_soc",
        )